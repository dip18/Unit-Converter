...
#Currency Conversion
def currency():
    amount_input = str(input("Enter amount to convert: "))
    from_currency = str(input("Enter your source currency (First 3 digit code in Capital): "))
    to_currency = str(input("Enter the currence you would like to convert to (First 3 digit code in Capital): "))
    request = urlopen('https://www.amdoren.com/api/currency.php?api_key=i9yGiqbK579jBt4dEhbusXPjcyGWHw&from=' + from_currency + '&to=' + to_currency + '&amount=' + amount_input)
    response = json.loads(request.read())
    print("%s %s is equal to %f %s" % (amount_input, from_currency, float(response['amount']), to_currency))
